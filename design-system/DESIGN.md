...
- この CSS は Constructable Stylesheet として全スライドの Shadow DOM に
  共有適用される。**`@font-face` を書かない**(shadow 内では無効。フォント
  追加は `design-system/fonts.css` に書き、ドキュメント側で読む)
...
- **既存のトークン名とレイアウトクラス名を維持する**(decks/ 内の既存
...
| `layout-section` | セクション区切り。**配色を反転**(明るい背景+暗いインク)して切り替わりを体感させる | — |
| `layout-two-col` | `.cols` を 2 等分グリッド、gap 56px、残り高さいっぱい | `.cols` 直下に `<div>` ×2 |
...
- [ ] `decks/demo` の 6 枚が崩れずに表示される(規約のリファレンスデッキ)
