...
プレビューは `preview` skill を使う(ローカルサーバーで見るか、スマホなど
向けに Artifact として束ねるかは skill 側が状況に応じて判断する)。

公開は GitHub Pages(Settings → Pages → Source を「Deploy from a branch」、
main / root に設定)。**テンプレートからリポジトリを作成しても Pages の設定
は引き継がれないため、デッキごとに手動設定が必要。** 設定後はビルドはなく、
main への push でリポジトリ root がそのまま配信される(ルートの
`.nojekyll` で Jekyll の加工も無効化済み)。プレビューと配信物が同一なので、
ローカルで動けば公開後も動く。**Pages の URL 直下を開くとそのまま上映**が
始まり、各スライドは `slides/NN-slug.html` で単体閲覧もできる。
