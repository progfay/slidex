...
リポジトリルートの `draft.md` に発表内容をまず雑多に書き出す。構成やスライド
単位を気にする必要はなく、思いつく要点・語り・素材を並べるだけでよい。
...
`draft.md` の素材の取捨選択・並べ替えはここで行う。確定したらユーザーに
確認を取る(ここから先は HTML 化のコストがかかるので、直すなら今が安い)。
