...
2. `slides/` のデモスライドと `assets/placeholder.svg` を削除する
   (`assets/favicon.svg` はビューアが使うので残す)
...
- **相対URLはすべてリポジトリルート基準**で書き、`<head>` の先頭付近に必ず
  `<base href="../">` を置く。単体表示では `<base>` が、シェル経由では
  ルートの index.html がドキュメント基準になるため、同じパスが両モードで解決できる
  (`<base>` はシェル取り込み時に捨てられる)。CSS の `url()` も同じ基準で書ける
- **画像などのアセットは `assets/` に置き**、`assets/foo.png` のように参照する
...
- 画像などのアセットは `assets/` に置く(参照パスは「スライドHTMLの契約」を参照)
